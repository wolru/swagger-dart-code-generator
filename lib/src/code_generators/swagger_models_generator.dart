...
abstract class SwaggerModelsGenerator extends SwaggerGeneratorBase {
  ...
  String generateModelClassContent(
    ...
    if (options.overridenModels.firstWhereOrNull((e) => e.fileName == fileName)?.overridenModels.contains(getValidatedClassName(className)) == true) {
      return '';
    }
    ...
    if (schema.type == 'array') {
      ...
      if (items != null) {
        ...
        if (options.overridenModels.firstWhereOrNull((e) => e.fileName == fileName)?.overridenModels.contains(getValidatedClassName(itemClassName)) == true) {
          return '';
        }
  ...
  Map<String, SwaggerSchema> getClassesFromInnerClasses(
    ...
    classes.forEach((classKey, schema) {
      ...
      properties.forEach((propertyKey, propSchema) {
        ...
        final innerClassName = getValidatedClassName('${getValidatedClassName(classKey)}$itemPart${getValidatedClassName(propertyKey)}');
      ...
      if (schema.items != null) {
        result.addAll(getClassesFromInnerClasses({'${getValidatedClassName(classKey)}\$item': schema.items!}));
      }
    });
  ...
  Map<String, SwaggerSchema> getClassesFromResponses(SwaggerRoot root) {
    ...
    paths.forEach((key, path) {
      path.requests.forEach((operation, request) {
        ...
        if (neededSchema != null && neededSchema.type == kObject && neededSchema.properties.isNotEmpty) {
        ...
        } else if (neededSchema != null && neededSchema.title.isNotEmpty && neededSchema.allOf.isNotEmpty) {
  ...
  String generateBase({
    ...
    final allEnumsString = generateEnumsMethods ? allEnums.map((e) => e.generateFromJsonToJson(options.enumsCaseSensitive)).join() : '';
  ...
  String getParameterTypeName(
    ...
    if (parameter.items?.properties.isNotEmpty == true) {
      final parameterNameCombination = '${getValidatedClassName(className)}\$${getValidatedClassName(parameterName)}\$Item$modelPostfix';
...
''';
  ...
  String generatePropertyContentByDefault({
    ...
    if (typeName != kDynamic && (prop.shouldBeNullable || options.nullableModels.contains(typeName))) {
  ...
  JsonEnumValue generateEnumValue({
    ...
    if (validatedTypeName.startsWith('enums.')) {
      ...
      jsonKey = ', toJson: $enumNameCamelCase${isNullable && !isList ? 'Nullable$toJsonSuffix' : toJsonSuffix}, fromJson: $fromJsonFunction, $defaultValueSuffix';
...
static $returnType $fromJsonFunction($valueType? value) => $enumNameCamelCase$fromJsonSuffix(value, $defaultValueString);
  ...
  String generatePropertyContentByAllOf({
    ...
    if (allOf.where((element) => element.ref.isNotEmpty || element.properties.isNotEmpty).length > 1) {
      typeName = kDynamic;
  ...
  String generatePropertyContentByRef(
    ...
    if (kBasicSwaggerTypes.contains(refSchema?.type) && allClasses[getValidatedClassName(parameterName)]?.isEnum != true) {
    ...
    if (propertySchema?.shouldBeNullable == true || isPropertyNullable || options.nullableModels.contains(className)) {
      typeName = typeName.makeNullable();
    }
    ...
    if (items != null) {
      ...
      if (typeName.isEmpty) {
        if (items.hasRef) {
          ...
          if (!allEnumListNames.contains(typeName) && !allEnumNames.contains(typeName) && !basicTypesMap.containsKey(typeName)) {
            typeName += options.modelPostfix;
          }
        }
